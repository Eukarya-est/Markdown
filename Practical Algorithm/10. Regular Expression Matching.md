...
private:
    bool solve(int i, int j, const string& s, const string& p, vector<vector<int>>& memo) {
        ...
        const bool firstMatch = (i < s.length() && (p[j] == s[i] || p[j] == '.'));
        ...
        if (j + 1 < p.length() && p[j + 1] == '*') {
            ...
            res = firstMatch && solve(i + 1, j + 1, s, p, memo);
        }
...
func isMatch(s string, p string) bool {
    ...
    solve = func(i, j int) bool {
        ...
        var res bool
        if j + 1 < n && p[j + 1] == '*' {
            res = solve(i, j + 2) || (firstMatch && solve(i + 1, j))
        } else {
            res = firstMatch && solve(i + 1, j + 1)
        }
...
class Solution:
    def isMatch(self, s: str, p: str) -> bool:
		...
		#Decorator 'cache' automatically stores the results of (i, j)
        @cache
