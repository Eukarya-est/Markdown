...
class Solution:
    def longestPalindrome(self, s: str) -> str:
        ...
        while i < n:
            if (n - i) * 2 <= max_len: break

            l, r = i, i
            
            while r < len(s) - 1 and s[r + 1] == s[i]: r += 1
...
class Solution {
 ...
 private:
  ...
  vector<int> manacher(const string& t) {
    ...
    int center = 0;
    int rightBoundary = 0;
    for (int i = 1; i < t.length() - 1; ++i) { // Skip the sentinel characters @ and $.
...
func longestPalindrome(s string) string {
    ...
    bestCenter, maxPalindromeLength := -1, 0

    for i := 1; i < n - 1; i++ {
        mirrorIndex := center - (i - center)

        if rightBoundary > i {p[i] = min(rightBoundary - i, p[mirrorIndex])}
...
func join(s string) string {
    ...
    joined.Grow(len(s) * 2 + 3)

    joined.WriteByte('@')
    for i := 0; i < len(s); i++ {
        joined.WriteByte('#')
        joined.WriteByte(s[i])
    }
    joined.WriteByte('#')
    joined.WriteByte('$')
...
class Solution:
    def longestPalindrome(self, s: str) -> str:
        ...
        best_center, max_palindrome_length = 0, 0

        for i in range(1, n - 1):
            mirror_index = center - (i - center)
            if right_boundary > i: p[i] = min(right_boundary - i, p[mirror_index])

            while t[i + p[i] + 1] == t[i - p[i] - 1]: p[i] += 1
            
            if i + p[i] > right_boundary: center, right_boundary = i, i + p[i]

            if p[i] > max_palindrome_length:
                max_palindrome_length, best_center = p[i], i
